...
const storageKeyForResource = (resourceName) => String(resourceName)
const resourceNameWithStorage = (storageKey) => String(storageKey)
...
class MemoryDriver extends storageMix(Driver) {
  ...
  one (resourceName, id) {
    const s = this
    let storageKey = storageKeyForResource(resourceName)
    return co(function * () {
  ...
  resources () {
    ...
    return co(function * () {
      ...
      return storageKeys.map((storageKey) => ({
        name: resourceNameWithStorage(storageKey),
