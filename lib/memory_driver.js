...
class MemoryDriver extends MemoryDriverBase {
  ...
  create (resourceName, attributes) {
    ...
    const storageKey = s.storageKeyToResourceName(resourceName)
    return co(function * () {
      const storage = yield s.getStorage(storageKey)
      const count = Object.keys(storage).length + 1
      const {id = clayId()} = attributes
      const created = Object.assign({}, {id, $$num: count}, attributes)
  ...
  resources () {
    const s = this
    return co(function * () {
      const storageKeys = yield s.storageKeys()
